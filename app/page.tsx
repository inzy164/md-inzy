  ...
  const analyze = () => {
    ...
    setTimeout(() => {
      ...
      setResult({
        ...
        advice: score < 75 ? "Significant buildup detected. Deep cleanse, exfoliate twice weekly, use Vitamin C serum and daily moisturizer with SPF 50." : score < 85 ? "Light pollution buildup detected. Double cleanse in the evening and add Vitamin C serum for brightness." : "Excellent! Your skin is in perfect condition. Continue your routine with daily cleansing and SPF protection."
      })
      setScanning(false)
    }, 2200)
  }
  ...
  const handleUpload = (e: any) => {
    ...
    setTimeout(() => {
      ...
      setResult({
        dustLevel: score > 85 ? "Low" : score > 70 ? "Medium" : "High",
        score,
        issues: score < 75 ? ["Clogged Pores", "Dullness", "Pollution Layer", "Oil & Dust Mix"] : ["Skin is Clean"],
        advice: score < 75 ? "Significant buildup detected. Deep cleanse, exfoliate twice weekly, use Vitamin C serum and daily moisturizer with SPF 50." : "Excellent! Your skin is in perfect condition. Continue your routine."
      })
    }, 1500)
  }
  ...
  return (
    <div style={{minHeight:"100vh", background:"#0a0a0a", color:"white", display:"flex", flexDirection:"column", alignItems:"center", padding:"20px", fontFamily:"system-ui"}}>
      <div style={{marginTop:"20px", textAlign:"center"}}>
        <div style={{display:"inline-block", background:"rgba(255,255,255,0.1)", border:"1px solid rgba(255,255,255,0.1)", padding:"4px 10px", borderRadius:"20px", fontSize:"11px", letterSpacing:"1px"}}>⚡ AI-POWERED • INZY164</div>
        <h1 style={{fontSize:"42px", fontWeight:"800", margin:"12px 0 0 0", letterSpacing:"-1px"}}>FaceScan AI</h1>
        <p style={{color:"#888", fontSize:"13px", marginTop:"6px"}}>Dust & Pollution Damage Analyzer • Professional Edition</p>
      </div>
      ...
      <div style={{width:"100%", maxWidth:"380px", marginTop:"20px", background:"#151515", border:"1px solid #2a2a2a", borderRadius:"24px", padding:"12px"}}>
        {mode==="live" ? (
          <div style={{position:"relative", borderRadius:"18px", overflow:"hidden", background:"black", height:"420px"}}>
            ...
            {scanning && <div style={{position:"absolute", inset:0, background:"rgba(0,0,0,0.7)", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center"}}><div style={{width:"36px", height:"36px", border:"3px solid #333", borderTop:"3px solid white", borderRadius:"50%"}}></div><p style={{marginTop:"12px", fontSize:"13px"}}>Scanning Face...</p></div>}
          </div>
        ) : (
        ...
        {result && (
          <div style={{marginTop:"14px", background:"black", border:"1px solid #222", borderRadius:"16px", padding:"14px"}}>
            ...
            <p style={{marginTop:"14px", fontSize:"12px", background:"#1a1a1a", padding:"10px", borderRadius:"10px", border:"1px solid #222", marginBottom:0, lineHeight:"16px"}}>💡 <b>Advice:</b> {result.advice}</p>
          </div>
        )}
      </div>
      <p style={{fontSize:"10px", color:"#444", marginTop:"20px", letterSpacing:"2px"}}>POWERED BY INZY164 • ENGLISH VERSION</p>
    </div>
  )
}
